<div class="grid" style="display: grid;grid-template-columns: 30% 69%" markdown>

<div class="grid cards" markdown>

-   :material-tooltip-edit:{ .lg .middle } __About__

!!! pied-piper ""
    ![website](../assets/images/Cover.png)

    ---

    - [x] Hey, I'm [YuiKuen.Yuen](https://bento.me/yuen){target=“_blank”}!
    - [x] In Learning..
        * [x] System
        * [x] Network
        * [x] Database
        * [x] Virtualization
        * [x] Cloud Computing
        * [x] DevOps
        * [x] Security Monitoring
        * [ ] AI

</div>

<div class="grid cards" style="display: grid; grid-template-columns: 1fr;" markdown>

-   :material-star-shooting:{ .lg .middle } __Recommendations__

    ---

    :material-eye-check:{ .lg .middle } [Yuen's Info Silo](http://17121203.xyz/)

    :material-numeric-1-box:{ .lg .middle } [111](111) - 1111

    :material-numeric-2-box:{ .lg .middle } [222](222) - 2222

<div class="grid cards" style="display:grid; grid-template-columns: 49% 49% !important;" markdown>


-   :material-file-link:{ .lg .middle } __Release__

    ---

    更新中...

    [→ 所有版本](https://github.com/Wcowin/Wcowin.github.io)


-   :material-flag-variant-minus:{ .lg .middle } __Test__

    ---

    更新中...

    [→ 了解更多](https://github.com/Wcowin/Wcowin.github.io)

</div>

</div>

</div>




